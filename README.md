...
(safe-docx  (fastModel) (fastModel)(writeModel)(fastModel)(fastModel)(fixModel)
 优先读取)
```
...
- **可配置模型路由**: 机械任务用 `fastModel`（默认 haiku），撰写用 `writeModel`，修复用 `fixModel`
...
```bash
# 1. 克隆项目
git clone https://github.com/hookrise/academic-editor-workflow.git
cd academic-editor-workflow

# 2. 安装 workflow（二选一）
# 方式 A: 用户级安装
cp workflows/academic-editor-workflow.js ~/.claude/workflows/
...
```bash
# 新建短文（lite 模式）
/academic-editor-workflow task="写一篇关于大语言模型提示工程的综述" mode=lite totalWordTarget=2000

# 标准学术文章（full 模式）
/academic-editor-workflow task="Transformer 架构在计算机视觉中的应用综述" mode=full

# 深度论文（ultra 模式）
/academic-editor-workflow task="多模态大语言模型的幻觉问题：成因、检测与缓解" mode=ultra totalWordTarget=8000

# 编辑已有文档
/academic-editor-workflow task="补充参考文献、修正实验数据、改进学术语言" targetFile="C:\docs\paper-draft.docx"
```
...
|### 模型覆盖
|
|默认机械性任务（搜索、审查、格式化、规划）使用 `fastModel`（默认 haiku），撰写和修复使用 CLI 默认模型。可通过参数覆盖：
|
|```bash
/academic-editor-workflow task="..." fastModel=sonnet writeModel=claude-sonnet-4-20250514
|```
|
|| 参数 | 默认值 | 说明 |
||------|--------|------|
|| `fastModel` | `haiku` | 搜索/审查/格式化/规划/大纲/瘦身 |
|| `writeModel` | CLI 默认 | 撰写（不指定则用 `--model` 值） |
|| `fixModel` | CLI 默认 | 修复（不指定则用 `--model` 值） |
### 模式对比
